...
export class AudioCaptureWindows extends EventEmitter {
  private logger = new Logger('AudioCapture');
  ...
  async startCapture(callId: string): Promise<void> {
    if (this.isCapturing) {
    ...
    try {
      ...
      this.captureProcess = spawn(soxPath, args, {
...
export default AudioCaptureWindows;