...
export interface WSEvents {
  // Client -> Server
  'agent:connect': { agentId: string; email: string; extension: string };
  'agent:disconnect': { agentId: string };
  'call:start': { call: Call3CX };
  'call:end': { callId: string };
  'audio:chunk': AudioChunk;
  'ticket:create': { ticket: NinjaTicket; callId: string };
  'ticket:update': { ticketId: number; updates: Partial<NinjaTicket> };
  
  // Server -> Client
  'transcription:partial': RealtimeTranscription;
  'transcription:final': Transcription;
  'analysis:complete': CallAnalysis;
  'agent:status': { agents: Agent[] };
  'call:update': Call3CX;
